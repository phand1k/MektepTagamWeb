...
namespace MektepTagamWeb.Areas.Identity.Pages.Account
{
    public class RegisterModel : PageModel
    {
        ...
        public async Task<IActionResult> OnPostAsync(string returnUrl = null)
        {
            ...
            {
                var checkOrganizationExists = await context.Organizations.
                    Where(x=>x.Number == Input.OrganizationNumber && x.IsDeleted == false).FirstOrDefaultAsync();
                if (checkOrganizationExists != null)
                {
                    ...
                    user.IndividualNumber = Input.IndividualNumberUser;
                    var phoneNumber = new String(Input.Email.Where(Char.IsDigit).ToArray());
                    user.PhoneNumber = phoneNumber;
                    await _userStore.SetUserNameAsync(user, phoneNumber, CancellationToken.None);
                    await _emailStore.SetEmailAsync(user, phoneNumber, CancellationToken.None);
                    ...
                    if (result.Succeeded)
                    {
                        ...
                        cardCode.AspNetUserId = userId;
                        cardCode.Code = userId;
                ...
                ModelState.AddModelError("Input.OrganizationNumber", "Неверный номер организации или организация приостановлена");
                return Page();
            }
